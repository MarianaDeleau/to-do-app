...
const TaskCard: FC<Props> = ({ id, title, category, description, progress, creationDate, startDate, completionDate }) => {
    ...
    <Card style={{ width: '15rem' }} className="cardTask shadow-lg p-3 mb-5 bg-body rounded" data-id={id} data-progress={progress}>
        <Card.Body>
            <Card.Title className="fw-bold h5">📌{title}</Card.Title>
                ...
                <Card.Subtitle className="mb-1 fw-light fs-6"><span className="badge rounded-pill bg-warning text-light text-wrap">{category}</span></Card.Subtitle>
                ...
                <Link className="text-decoration-none linkButton" to={`./edit-task?id=${id}`}>Editar Tarea</Link><span className="badge rounded-pill bg-danger text-light ms-1">{progress}</span>
        </Card.Body>
    </Card>            
    )
}
